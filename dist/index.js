...
logger_1.logger.info('BOOT', 'index initialization started');
function validateEnv() {
    ...
    const missing = Object.entries(env)
        ...
        .map(([key]) => key);
    if (missing.length > 0) {
        logger_1.logger.error('ENV', 'Missing required environment variables', { missing });
        (0, logger_1.updateHealth)(s => s.discord.lastError = `Missing required environment variables: ${missing.join(', ')}`);
        process.exit(1);
    }
    logger_1.logger.info('ENV', 'Environment validation passed', {
...
const webhook = new discord_js_1.WebhookClient({ url: WEBHOOK_URL });
(0, logger_1.startHeartbeat)();
...
function logSchemaSafeInsertFailure(table, payload, error) {
    const payloadKeys = Object.keys(payload);
    const scope = `DB][${table}][insert`;
    logger_1.logger.error(scope, 'insert failed', {
        table,
        payloadKeys,
...
async function safeEditReply(interaction, content, context) {
    try {
        ...
        logger_1.logger.info('INTERACTION', `reply success: ${context}`);
        (0, logger_1.updateHealth)(h => {
            h.interactions.lastInteractionAt = new Date().toISOString();
        });
    }
    catch (replyError) {
        logger_1.logger.error('INTERACTION', `reply failed: ${context}`, replyError);
        (0, logger_1.updateHealth)(h => h.discord.lastError = `reply_failed:${context}:${replyError?.message || 'unknown'}`);
    }
}
function extractWeeklyReport(jypText) {
    if (!jypText)
        return null;
...
function getThisWeekFridayKST(now = new Date()) {
...
function shouldGenerateWeeklyReport(now, targetFridayKst, lastReportDate) {
    if (now < targetFridayKst)
        return false;
    if (!lastReportDate)
        return true;
...
async function runWeeklyReportSchedulerCheck() {
    ...
    try {
        const { data, error } = await supabase
            ...
            .limit(1);
        if (error) {
            logger_1.logger.error('SCHEDULER', 'weekly report last-date query failed', error);
            return;
        }
        const lastReportDate = data && data.length > 0 && data[0].created_at ? new Date(data[0].created_at) : null;
        logger_1.logger.info('SCHEDULER', 'weekly report check', {
            now: now.toISOString(),
        ...
        if (!shouldGenerateWeeklyReport(now, targetFriday, lastReportDate)) {
            logger_1.logger.warn('SCHEDULER', 'weekly report skipped');
            return;
        }
        const jyp = new agents_1.JYPAgent();
        ...
        if (insertError) {
            logger_1.logger.error('SCHEDULER', 'weekly report insert failed', {
        ...
        logger_1.logger.info('SCHEDULER', 'weekly report generated');
    }
    catch (e) {
        logger_1.logger.error('SCHEDULER', 'weekly report scheduler error', e);
    }
}
function startWeeklyReportScheduler() {
    if (globalThis.__weeklySchedulerStarted) {
        return;
    }
    globalThis.__weeklySchedulerStarted = true;
    logger_1.logger.info('SCHEDULER', 'weekly report scheduler started');
...
function isTrendQueryCheck(query) {
    return /(트렌드|유행|k-?pop|드라마|넷플릭스|스포츠|콘텐츠|아이돌|엔터|기회|시장)/i.test(query);
}
async function broadcastAgentResponse(userId, agentName, avatarURL, content, sourceInteraction) {
    let finalContent = content;
    let components = [];
    if (finalContent.includes('[REASON: NO_DATA]')) {
        finalContent = finalContent.replace(/\[REASON: NO_DATA\]/g, '').trim();
        components = [(0, panelManager_1.getNoDataButtons)()];
    }
    ...
    try {
        ...
        logger_1.logger.error('DISCORD', `Webhook send error: ${e.message}`, e);
        if (sourceInteraction && sourceInteraction.channel) {
            await sourceInteraction.channel.send({
                content: `**[${agentName}]**\n${finalContent}`,
                components: components.length > 0 ? components : undefined
            });
        }
...
async function checkDataGate(userId) {
    try {
        ...
        const validation = (0, agents_1.hasRequiredAnchoredData)({
        ...
        logger_1.logger.error('GATE', 'Data gate checking crashed', e);
        return false;
    }
}
async function runDebate(userId, userQuery, sourceInteraction) {
    try {
        const isTrendQuery = isTrendQueryCheck(userQuery);
        const isDataOk = await checkDataGate(userId);
        logger_1.logger.info('ROUTER', `isTrend=${isTrendQuery}`);
        (0, logger_1.updateHealth)(s => s.ai.lastRoute = isTrendQuery ? 'trend' : 'financial');
        if (!isTrendQuery && !isDataOk) {
            logger_1.logger.info('GATE', 'NO_DATA triggered');
            logger_1.logger.info('AI', 'Gemini skipped due to NO_DATA');
            (0, logger_1.updateHealth)(s => s.ai.lastNoDataTriggered = true);
            ...
            if (sourceInteraction.isButton?.() || sourceInteraction.isModalSubmit?.()) {
                await sourceInteraction.followUp({ embeds: [embed], components: [(0, panelManager_1.getNoDataButtons)()] });
            }
            else {
                await sourceInteraction.reply({ embeds: [embed], components: [(0, panelManager_1.getNoDataButtons)()] });
            }
            return;
        }
        (0, logger_1.updateHealth)(s => s.ai.lastNoDataTriggered = false);
        logger_1.logger.info('AI', 'Gemini call started');
        const ray = new agents_1.RayDalioAgent();
        const jyp = new agents_1.JYPAgent();
        const simons = new agents_1.JamesSimonsAgent();
        const drucker = new agents_1.PeterDruckerAgent();
        const cio = new agents_1.StanleyDruckenmillerAgent();
        await Promise.all([
        ...
        if (rayRes?.includes("[REASON: NO_DATA]") && !isTrendQuery) {
            logger_1.logger.warn('AI', 'Ray Dalio aborted due to NO_DATA at logic layer');
            return;
        }
        ...
        logger_1.logger.info('AI', 'Gemini call completed');
        ...
        logger_1.logger.info('DB', 'chat_history payload preview', {
            keys: Object.keys(chatHistoryPayload),
            hasWeeklyReport: !!weeklyReport
        });
        logger_1.logger.info('DB', 'chat_history insert attempt', {
            keys: Object.keys(chatHistoryPayload)
        });
        const { error: dbError } = await supabase.from('chat_history').insert(chatHistoryPayload);
        if (dbError) {
            logger_1.logger.error('DB', '[chat_history][insert] failed', {
            ...
            logger_1.logger.info('DB', 'chat_history insert success');
        }
    }
    catch (err) {
        logger_1.logger.error('ROUTER', "토론 에러: " + err.message, err);
    }
}
const commandQueryMap = {
...
client.on('messageCreate', async (message) => {
    ...
    if (message.content.startsWith('!')) {
        logger_1.logger.info('COMMAND', `message command received: ${message.content}`, { user: message.author.tag });
    }
    if (message.content === '!메뉴' || message.content === '!패널재설치') {
        const msg = await message.reply((0, panelManager_1.getMainPanel)());
        (0, panelManager_1.savePanelState)(msg.channel.id, msg.id);
        logger_1.logger.info('PANEL', 'Explicit reinstall via text command', { channelId: msg.channel.id, messageId: msg.id });
        (0, logger_1.updateHealth)(s => s.panels.lastPanelAction = 'manual_reinstall');
        return;
    }
    if (message.content.startsWith('!종목추가')) {
        const parts = message.content.split(' ');
        if (parts.length < 4)
            return message.reply("❌ 사용법: `!종목추가 [심볼] [수량] [평단가] [종목명?] [섹터?]`");
        ...
        if (qty === null || price === null)
            return message.reply("❌ 수량과 평단가는 숫자로 입력해주세요.");
        ...
        if (error) {
            logger_1.logger.error('DATABASE', 'Supabase insert failure', error);
    ...
    if (message.content.startsWith('!현금흐름추가')) {
        const parts = message.content.split(' ');
        if (parts.length < 3)
            return message.reply("❌ 사용법: `!현금흐름추가 [종류] [금액] [설명...]` (income, fixed_expense, saving, investment, debt_payment, other)");
        const [_, typeRaw, amountStr, ...descParts] = parts;
        const flowType = normalizeFlowType(typeRaw);
        if (!flowType)
            return message.reply("❌ 잘못된 현금흐름 종류입니다. (income, fixed_expense, saving, investment, debt_payment, other 중 택 1)");
        const amount = parseNumberStrict(amountStr);
        if (amount === null)
            return message.reply("❌ 금액은 숫자로 입력해주세요.");
...
client.on('interactionCreate', async (interaction) => {
    try {
        if (interaction.isButton()) {
            ...
            logger_1.logger.info('INTERACTION', `button customId=${cid}`, { user: interaction.user.tag });
            ...
            if (cid.startsWith('panel:main:')) {
                if (cid === 'panel:main:portfolio')
                    await interaction.update((0, panelManager_1.getPortfolioPanel)());
                else if (cid === 'panel:main:finance')
                    await interaction.update((0, panelManager_1.getFinancePanel)());
                else if (cid === 'panel:main:ai')
                    await interaction.update((0, panelManager_1.getAIPanel)());
                else if (cid === 'panel:main:trend')
                    await interaction.update((0, panelManager_1.getTrendPanel)());
                else if (cid === 'panel:main:settings')
                    await interaction.update((0, panelManager_1.getSettingsPanel)());
                else if (cid === 'panel:main:reinstall')
                    await interaction.update((0, panelManager_1.getMainPanel)());
                return;
            }
            if (cid === 'panel:settings:reinstall') {
                const msg = await interaction.channel?.send((0, panelManager_1.getMainPanel)());
                if (msg)
                    (0, panelManager_1.savePanelState)(msg.channel.id, msg.id);
                logger_1.logger.info('PANEL', 'Explicit reinstall via button', { channelId: msg?.channel.id, messageId: msg?.id });
                (0, logger_1.updateHealth)(s => s.panels.lastPanelAction = 'button_reinstall');
            ...
            if (cid === 'panel:portfolio:add') {
                const modal = new discord_js_1.ModalBuilder().setCustomId('modal:portfolio:add').setTitle('📈 신규 종목 등록');
                modal.addComponents(new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder().setCustomId('symbol').setLabel("심볼 (티커)").setStyle(discord_js_1.TextInputStyle.Short)), new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder().setCustomId('qty').setLabel("수량").setStyle(discord_js_1.TextInputStyle.Short)), new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder().setCustomId('price').setLabel("평균 매수 단가").setStyle(discord_js_1.TextInputStyle.Short)));
                await interaction.showModal(modal);
                return;
            }
            if (cid === 'panel:finance:add_expense') {
                const modal = new discord_js_1.ModalBuilder().setCustomId('modal:expense:add').setTitle('💸 지출 기록');
                modal.addComponents(new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder().setCustomId('amount').setLabel("금액").setStyle(discord_js_1.TextInputStyle.Short)), new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder().setCustomId('category').setLabel("카테고리").setStyle(discord_js_1.TextInputStyle.Short)), new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder().setCustomId('desc').setLabel("상세 설명").setStyle(discord_js_1.TextInputStyle.Paragraph).setRequired(false)));
                await interaction.showModal(modal);
                return;
            }
            if (cid === 'panel:finance:add_cashflow') {
                logger_1.logger.info('INTERACTION', 'button click: panel:finance:add_cashflow', { user: interaction.user.tag });
                const modal = new discord_js_1.ModalBuilder().setCustomId('modal:cashflow:add').setTitle('💰 현금흐름 입력');
                modal.addComponents(new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder().setCustomId('flow_type').setLabel("종류 (income/saving/investment/fixed_expense 등)").setStyle(discord_js_1.TextInputStyle.Short)), new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder().setCustomId('amount').setLabel("금액").setStyle(discord_js_1.TextInputStyle.Short)), new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder().setCustomId('desc').setLabel("상세 설명").setStyle(discord_js_1.TextInputStyle.Paragraph).setRequired(false)));
                logger_1.logger.info('INTERACTION', 'modal open: modal:cashflow:add', { user: interaction.user.tag });
                await interaction.showModal(modal);
                return;
            }
            if (cid === 'panel:ai:ask' || cid === 'panel:trend:free') {
                const modal = new discord_js_1.ModalBuilder().setCustomId(cid === 'panel:ai:ask' ? 'modal:ai:ask' : 'modal:trend:free').setTitle(cid === 'panel:ai:ask' ? '✍️ 직접 질문' : '🔍 자유 탐색');
                modal.addComponents(new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.TextInputBuilder().setCustomId('query').setLabel("궁금한 내용을 입력하세요").setStyle(discord_js_1.TextInputStyle.Paragraph)));
                await interaction.showModal(modal);
                return;
            }
            if (cid === 'panel:portfolio:view') {
                await interaction.deferReply({ ephemeral: true });
                const { data, error } = await supabase.from('portfolio').select('*').eq('user_id', interaction.user.id);
                if (error) {
                    logger_1.logger.error('DATABASE', 'Supabase select failures', error);
                    await safeEditReply(interaction, "❌ DB 조회 실패", 'panel:portfolio:view:db_failure');
                    return;
                }
                if (!data || data.length === 0) {
                    await safeEditReply(interaction, "보유 중인 종목이 없습니다.", 'panel:portfolio:view:empty');
                }
                else {
        ...
        if (interaction.type === discord_js_1.InteractionType.ModalSubmit) {
            const cid = interaction.customId;
            logger_1.logger.info('INTERACTION', `modal customId=${cid}`, { user: interaction.user.tag });
            (0, logger_1.updateHealth)(s => {
                s.interactions.lastInteractionAt = new Date().toISOString();
            ...
            if (cid === 'modal:portfolio:add') {
                ...
                if (!qty || !price)
                    return interaction.reply({ content: "❌ 수량과 평단가는 0보다 큰 숫자여야 합니다.", ephemeral: true });
            ...
            if (cid === 'modal:cashflow:add') {
                logger_1.logger.info('INTERACTION', 'modal submit received: modal:cashflow:add', { user: interaction.user.tag });
                await interaction.deferReply({ ephemeral: true });
                try {
                    ...
                    if (!flowType || !amount) {
                        logger_1.logger.warn('INTERACTION', 'validation failure: modal:cashflow:add', {
                            flowTypeRaw,
                    ...
                    logger_1.logger.info('INTERACTION', 'validation success: modal:cashflow:add', { flowType, amount });
                    const payload = {
                    ...
                    logger_1.logger.info('DB', '[cashflow][insert] attempt', { table: 'cashflow', payloadKeys: Object.keys(payload) });
                    ...
                    logger_1.logger.info('DB', '[cashflow][insert] success', { userId: interaction.user.id, flowType, amount });
                    await safeEditReply(interaction, "현금흐름이 정상적으로 저장되었습니다.", 'modal:cashflow:add:success');
                }
                catch (modalError) {
                    logger_1.logger.error('INTERACTION', 'modal:cashflow:add exception', modalError);
        ...
        logger_1.logger.error('INTERACTION', "Interaction Error: " + error.message, error);
    }
});
// Use clientReady hook to prevent warning & ensure safe boot
client.once(discord_js_1.Events.ClientReady, async (c) => {
    logger_1.logger.info('DISCORD', `ready bot=${c.user.tag} guilds=${c.guilds.cache.size}`);
    (0, logger_1.updateHealth)(s => {
    ...
    await (0, panelManager_1.restoreOrBuildMainPanel)(client);
    logger_1.logger.info('BOOT', `[🟢 PM2 Status] KJM Office Bot is Online: ${c.user.tag}`);
});
(0, logger_1.updateHealth)(s => s.discord.loginAttempted = true);
startWeeklyReportScheduler();
logger_1.logger.info('DISCORD', 'login attempt');
client.login(DISCORD_TOKEN).catch(e => {
    logger_1.logger.error('DISCORD', 'login failure', e);
    (0, logger_1.updateHealth)(s => s.discord.lastError = e.message);
});
