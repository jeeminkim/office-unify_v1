...
import { TradeJournalAnalyticsClient } from './TradeJournalAnalyticsClient';

export default async function TradeJournalAnalyticsPage() {
  ...
  if (!user) {
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-lg flex-col justify-center gap-4 p-8 text-slate-800">
        <h1 className="text-xl font-bold">Trade Journal Analytics</h1>
        <p className="text-sm text-slate-600">이 페이지는 Google 로그인 후 사용할 수 있습니다.</p>
        <GoogleSignInButton redirectTo="/trade-journal/analytics" />
  ...
  return <TradeJournalAnalyticsClient />;
}
