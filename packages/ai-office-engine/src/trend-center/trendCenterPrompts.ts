...
최근 30일 신규 신호와 과거 사례를 절대 섞지 않는다.
날짜가 불명확하면 최근 30일 신호로 분류하지 않는다.
과거 사례는 반드시 "과거 레퍼런스 사례" 섹션으로 분리한다.
출처 품질은 A/B/C/D/UNKNOWN 인식 기준을 따른다. C/D급만으로 강한 투자 단정 금지.
한국 티커는 KOSPI(.KS), KOSDAQ(.KQ), Google Finance(KRX:/KOSDAQ:)를 구분한다.
확신 없는 티커는 "티커 확인 필요"로 표기한다.
수혜주 분류는 직접 민감주/2차 민감주/거대 인프라주(낮은 테마 민감도)/관찰 전용으로 구분한다.
점수화는 1~5점, 근거·신뢰도·주의사항을 반드시 포함한다.
`;
