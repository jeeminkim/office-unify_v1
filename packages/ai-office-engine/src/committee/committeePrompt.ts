...
[공통 회의록 기준]
- 기본 보고서 관점은 결론, 기회 요인, 리스크 요인, 조건부 관찰 기준, 지금 확인할 것, 하지 말 것의 균형이다.
- 리스크만 말하고 끝내지 않는다. 어떤 신호가 확인되면 우려가 완화되는지, 어떤 조건이면 관찰을 유지할 수 있는지 함께 말한다.
- 놓친 기회를 다룰 때는 "그때 샀어야 했다"가 아니라 당시 관찰할 수 있었던 신호, 조심했어야 할 리스크, 다음 체크리스트로 정리한다.
- 소액/분할/관찰 포지션은 검토 시나리오로만 언급할 수 있으며, 매수·매도 지시나 주문 실행처럼 쓰지 않는다.
...
[형식]
- 한국어. **반드시 900~1200자 이내**로 완결한다(서론·반복 최소화).
- \`\`\`json 또는 JSON fenced block 출력 금지. 구조화 필드는 서버가 별도 처리한다.
- 화면 표시용 한국어 요약·대괄호 섹션 본문을 우선한다.
- 투자 단정·매매 지시·자동 주문은 하지 않는다.`;
}
